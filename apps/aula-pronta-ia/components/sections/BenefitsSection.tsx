    ...
    <section id="beneficios" className="py-32 px-4 sm:px-8 lg:px-16 bg-[#fafafa]" style={{ paddingTop: 0 }}>
      <div className="max-w-[1240px] mx-auto">
        ...
        <div className="grid grid-cols-6 auto-rows-[minmax(190px,auto)] gap-[18px]">
          ...
          <div className="col-span-6 sm:col-span-3 bg-gradient-to-br from-blue-50 to-white rounded-[18px] border border-slate-200 shadow-md p-7 card-hover hover:border-blue-200 hover:shadow-lg">
            ...
            <div
              ...
              style={{ fontSize: '60px', fontWeight: 380, fontVariationSettings: '"opsz" 144, "SOFT" 60' }}
            >
              <span className="gradient-text" style={{ fontStyle: 'italic', fontVariationSettings: '"opsz" 144, "SOFT" 100' }}>3h+</span>{" "}
              <span className="text-slate-900 text-3xl">por semana</span>
            </div>
            <h3 className="font-bold text-slate-900 text-[19px] tracking-[-0.02em] mt-3 mb-2">De volta para você</h3>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 border-t border-b border-slate-200 mt-12">
          {stats.map((s, i) => (
            <div key={s.n} className={`py-10 px-7 ${i < stats.length - 1 ? 'border-r border-slate-200' : ''}`}>
              <div
                className="font-fraunces text-blue-700 leading-none tracking-[-0.035em]"
                style={{ fontSize: '58px', fontWeight: 380, fontVariationSettings: '"opsz" 144, "SOFT" 60' }}
              >
                {s.n}
              </div>
              <div className="text-slate-500 text-[13.5px] mt-2 font-medium">{s.l}</div>
            </div>
          ))}
        </div>
